...
//         - 매개변수 앞에 해당 어노테이션을 작성하면 매개변수에 값이 주입됨
    ...
    // RedirectAttributes : redirect 시 데이터 전달 역할

    // * Forward VS RedirectAttributes *
    // forward : 서버 내부 이동 (요청 1번, 주소 유지)
    // Redirect : 브라우저에게 새 URL로 다시 요청하라고 지시(요청 2번, 주소 변경)

    // ra.addAttribute() : URL 쿼리 스트링으로 데이터 전달
    // ra.addFlashAttribute() : redirect 시 데이터를 request scope로 전달
    //                        -> session에 잠깐 저장 후 다시 request scope로 복원
    @GetMapping("/ex06")
...
//    }
    @GetMapping("/ex06-2")
    public String ex06_2(@ModelAttribute("dto") SampleDTO dto) {
        return "sample/ex06-2";
    }
